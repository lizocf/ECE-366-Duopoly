...
public class AccountDAO extends DataAccessObject<AccountUtil>
{
    ...
    private static final String UPDATE_WINS = "UPDATE accounts SET num_wins=? WHERE user_id=?";
    private static final String UPDATE_LOSS = "UPDATE accounts SET num_losses=? WHERE user_id=?";
    ...
    public AccountUtil update_dp(AccountUtil dto) { // update_dp
        try(PreparedStatement statement = this.connection.prepareStatement(UPDATE_DP);)
            {
                statement.setInt(1, 100); // 100 needs to be changed based on whether they win/lose/rank
                statement.setInt(2, dto.getUserId());
                statement.execute();
                return this.findById(dto.getUserId());
            } catch (SQLException e){
                e.printStackTrace();
                throw new RuntimeException(e);
            }
